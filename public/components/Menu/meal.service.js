...
const TRANSLATION_URL =
  "https://" + TRANSLATION_ENV + ".strakertranslations.com/v3/translate";
...
export async function postMeal(meal, newCatId) {
  ...
  meal.foodTypes = (meal.FoodTypes || meal.foodTypes || [])
    .map(x => x.Name || x.name)
    .join(",");
  meal.FoodTypes = meal.foodTypes;

  const {
    obj: [MealID]
  } = await Ajax().post("/meal", {
  ...
  const resImages = await ImageService.updateMealImages(
...
export async function updateMeal(meal, catId) {
  ...
  const resposne = await Ajax().put("/meal", {
    body: JSON.stringify(convertOpts(meal, true)),
    headers
  });
  // Delete removed images
  await Ajax().put("/meal-image-remove", {
    body: JSON.stringify({
      id: MealID,
      images: meal.images.filter(x => !x.newlyAdded)
    }),
    headers
  });
  const resImages = await ImageService.updateMealImages(
    meal.images
      .filter(x => x.newlyAdded)
      .map(image => {
        image.file.folder = "meal";
        return {
          catId: catId,
      ...
      return Ajax()
        ...
        .then(res => {
          ...
          let meals2 = meals.reduce((acc, current) => {
            return acc.concat(
              mealTranslations.reduce((acc_translation, curr_translation) => {
...
export function translateMeal(langs, meal) {
  return Ajax()
      ...
      return meals.find(m => {
        return (
          (m.Description === meal.description ||
            m.Description === meal.Description) &&
          (m.Title === meal.title || m.Title === meal.Title) &&
          (m.MenuCategoryID === meal.menuCategoryId ||
            m.MenuCategoryID === meal.MenuCategoryID)
        );
      }).MealID;
    })
    .then(id => {
      ...
      let propsToTranslate = Object.keys(meal)
        .filter(key => {
          return (
            (key === "title" ||
              key === "Title" ||
              (key === "description" || key === "Description")) &&
            (meal[key] && meal[key].length > 0)
          );
        })
        .map(key => {
          return {
          ...
          const translateLangs = (language, props, id) => {
            return props.map(prop => {
              return Ajax().post("/translate-meal", {
                body: JSON.stringify(
                  convertForTranslation(language, {
                    type: "meal",
                    id: id,
                    prop: prop
                  })
          ...
          if (!lang.name && !lang.title) {
            return Ajax()
              ...
              .then(res => {
                ...
                const currentBranchLang = branchLanguages.find(
                  l => l.BranchLanguageID === lang.branchLanguageId
                );
              ...
              .then(res => {
                ...
                const currentLanguage = languages.find(
                  l => l.LanguageID === currentBranchLangId
                );
                const finalLang = {
