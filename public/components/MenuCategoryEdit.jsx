...
class MenuCategoryEdit extends Component {
  ...
  render() {
    ...
    var calCategories = totalCategories
      ? JSON.parse(JSON.stringify(totalCategories))
      : [];

    var find = calCategories.find(cate => {
      return cate.CategoryStandardID == stateId;
    });
    ...
    const availableCategories = calCategories.map((cate, index) => {
      var match =
        cate.CategoryStandardID == stateId ||
        !categoriesAll.find(
          a => a.CategoryStandardID == cate.CategoryStandardID
        );
        ...
        <div className="food-menu-meals">
          <div className="branch--contact aside--section contacts--support">
            ...
            <div className="global-padding-wrapper">
              <div className="food-menu--meal">{mealComponents}</div>
            </div>
          </div>
        </div>
      </div>
